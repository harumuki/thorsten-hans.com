...
The third part of Azure Container Registry Unleashed will guide you through the process of integrating ACR and Azure Monitor to collect essential information and metrics about your ACR instances at runtime. The integration of ACR and Azure Monitor is currently in public preview. However, retrieving insights about your ACR instance is mission-critical, we will dive into this topic now.
...
title="Azure Monitor Overview" caption="Azure Monitor Overview" %}
...
title="Azure Monitor - Azure Resource Flow" caption="Azure Monitor - Azure Resource Flow" %}

To ship data from ACR to Azure Monitor, we’ve to utilize an Azure Log Analytics workspace. Chances are good that your subscription contains at least one Log Analytics, however we will create a new one to isolate data from ACR.
...
az monitor log-analytics workspace create -g acr-unleashed -n acr-unleashed-ws
...
You integrate ACR with Azure Monitor by adding a corresponding diagnostic setting to the ACR instance. Although this could be done via Azure Portal, we will use Azure CLI.
...
Once ACR and Azure Monitor are connected, it could take a couple of minutes until first logs and events show up in Azure Monitor. To populate some data, we will feed some Docker Images into the ACR instance. The following script will create a simple Docker Image and publish it under several tags in ACR.
...
{% include image-caption.html imageurl="/assets/images/posts/2019/acr-unleashed-azure-monitor.png"
